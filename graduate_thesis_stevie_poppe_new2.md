...
[^34]: I contrasted the term 'Netto-Uyoku' as a 'Topic' (which includes related keywords collected through neural networks) with the literal search terms '*netto-uyoku*' 「ネット右」 and '*neto-uyo*' 「ネトウヨ」. Interest indicates relative interest to the full amount of searches at that time, it is not an indicator of quantity. The highest value (100) represents the highest proportion of search results with all other results calculated in proportion. For more information on the methodology behind Google Trends, I refer to **\ref{appendix:googletrends}**.
